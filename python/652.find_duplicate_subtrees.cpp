...
https://www.youtube.com/watch?v=YupKiFqtnsA&t=206s
...
例如，对于根节点是node的子树，我们定义它的string key = to_string(node->val) + "#" + to_string(getId(node->left)) + "#" + to_string(getId(node->right))。其中getId就是将左子树（或者右子树）的序列化结果用映射的数字ID代表。显然通过这种方式生成的node的key，会比纯序列化的结果短很多。
